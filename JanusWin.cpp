...
#include <stdlib.h>
using namespace std;
const char MasterKey[] = "JanusKey1!46^";
struct Password {
	char textP[256] = {0};
	int lenP = 0 ; 
};
...
void decrpt(char* toDecrpt, const char* key, int len){
	for (int i = 0; i < len; i++){
		toDecrpt[i] = toDecrpt[i] ^ key[i % (strlen(key) / sizeof(char))];
}}
...
void AddUser() {
	...
	encrypt(user.username.Name, MasterKey, user.username.lenN);
	encrypt(user.password.textP, MasterKey, user.password.lenP);
	outf.write((const char*)(&user), sizeof(JanusUser));
	outf.close(); 
}
void ReadUser() {
	...
	if (file.is_open()) {
		...
		cout << user.username.Name << "\t" << user.password.textP << "\t";
		file.close();
		return; 
	}
	else {
...
bool auth(JanusUser temp) {
	decrpt(user.username.Name, MasterKey, user.username.lenN);
	decrpt(user.password.textP, MasterKey, user.password.lenP);
	if (strcmp(temp.password.textP, user.password.textP) == 0 && strcmp(temp.username.Name, user.username.Name) == 0) {
		return true; 
	}
	return false; 
}
...
public:
	...
	void deleteNode(Key* toDel) {
		...
		for (; temp != nullptr; temp = temp->next) {
			if (strcmp(temp->Data->App.Name,toDel->App.Name)==0 && strcmp(temp->Data->username.Name,toDel->username.Name)==0) {
				if (temp == head) {
					head = temp->next;
					free(temp->Data);
					free(temp);
